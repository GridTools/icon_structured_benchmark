...
template <typename T>
struct nabla4_vertical_interpolate_unstructured_inlined {
    nabla4_vertical_unstructured_gt<T> nabla4_data;
    interpolate_unstructured<T> interpolate_data;

    nabla4_vertical_interpolate_unstructured_inlined(std::vector<std::array<index_type, 4>> e2c2v,
...
template <typename T>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_kloop_vertical() {
    return {};
};

template <>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_kloop_vertical<std::size_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_kloop_vertical<std::int64_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_kloop_vertical<std::uint32_t>() {
    // not optimized
    return {32, 9, 1, 288};
};

template <>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_kloop_vertical<int>() {
    return {128, 4, 1, 512};
};

constexpr block_dims block_dims_unstructured_nabla_interpol_inlined_kloop_vertical =
    get_block_dims_unstructured_nabla_interpol_inlined_kloop_vertical<index_type>();

__global__ void __launch_bounds__(block_dims_unstructured_nabla_interpol_inlined_kloop_vertical.size)
    run_gpu_kloop_nabla4_vertical_interpolate_inlined_unstructured(index_type nabla4_output_size,
...
#pragma unroll
        for (auto i{0}; i < 6; ++i) {
            ...
            const double nabv_tang_wp =
                u_vert_gt_tv(E2C2V_0, k_index) * primal_normal_vert_v1_gt_tv(e2ecv[4 * i], k_index) +
                v_vert_gt_tv(E2C2V_0, k_index) * primal_normal_vert_v2_gt_tv(e2ecv[4 * i], k_index) +
                u_vert_gt_tv(E2C2V_1, k_index) * primal_normal_vert_v1_gt_tv(e2ecv[4 * i + 1], k_index) +
                v_vert_gt_tv(E2C2V_1, k_index) * primal_normal_vert_v2_gt_tv(e2ecv[4 * i + 1], k_index);
            const double nabv_norm_wp =
                u_vert_gt_tv(E2C2V_2, k_index) * primal_normal_vert_v1_gt_tv(e2ecv[4 * i + 2], k_index) +
                v_vert_gt_tv(E2C2V_2, k_index) * primal_normal_vert_v2_gt_tv(e2ecv[4 * i + 2], k_index) +
                u_vert_gt_tv(E2C2V_3, k_index) * primal_normal_vert_v1_gt_tv(e2ecv[4 * i + 3], k_index) +
                v_vert_gt_tv(E2C2V_3, k_index) * primal_normal_vert_v2_gt_tv(e2ecv[4 * i + 3], k_index);
...
template <typename T>
inline void nabla4_vertical_interpolate_unstructured_inlined<T>::run_gpu_kloop_helper() {
    dim3 tblocks(block_dims_unstructured_nabla_interpol_inlined_kloop_vertical.x,
        block_dims_unstructured_nabla_interpol_inlined_kloop_vertical.y,
        block_dims_unstructured_nabla_interpol_inlined_kloop_vertical.z);
    dim3 grid((interpolate_data.output_size + tblocks.x - 1) / tblocks.x, 1, 1);
    run_gpu_kloop_nabla4_vertical_interpolate_inlined_unstructured<<<grid, tblocks>>>(nabla4_data.output_size,
...
template <typename T>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_naive_vertical() {
    return {};
};

template <>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_naive_vertical<std::size_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_naive_vertical<std::int64_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_naive_vertical<std::uint32_t>() {
    // not optimized
    return {32, 9, 1, 288};
};

template <>
constexpr block_dims get_block_dims_unstructured_nabla_interpol_inlined_naive_vertical<int>() {
    return {32, 16, 1, 512}; // Not optimized
};

constexpr block_dims block_dims_unstructured_nabla_interpol_inlined_naive_vertical =
    get_block_dims_unstructured_nabla_interpol_inlined_naive_vertical<index_type>();

__global__ void __launch_bounds__(block_dims_unstructured_nabla_interpol_inlined_naive_vertical.size)
    run_gpu_naive_nabla4_vertical_interpolate_inlined_unstructured(index_type nabla4_output_size,
        index_type interpolate_output_size,
        index_type CellDim,
        index_type VertexDim,
        index_type EdgeDim,
...
template <typename T>
inline void nabla4_vertical_interpolate_unstructured_inlined<T>::run_gpu_naive_helper() {
    dim3 tblocks(block_dims_unstructured_nabla_interpol_inlined_naive_vertical.x,
        block_dims_unstructured_nabla_interpol_inlined_naive_vertical.y,
        block_dims_unstructured_nabla_interpol_inlined_naive_vertical.z);
    dim3 grid((interpolate_data.output_size + tblocks.x - 1) / tblocks.x,
        (interpolate_data.KDim + tblocks.y - 1) / tblocks.y,
        1);
    run_gpu_naive_nabla4_vertical_interpolate_inlined_unstructured<<<grid, tblocks>>>(nabla4_data.output_size,
        interpolate_data.output_size,
        nabla4_data.CellDim,
        interpolate_data.VertexDim,
        interpolate_data.EdgeDim,
