...
class mo_intp_rbf_rbf_vec_interpol_vertex {
    ...
    const data_store_2d_WP_t ptr_coeff_1_gt;
    const data_store_2d_WP_t ptr_coeff_2_gt;
    const data_store_2d_ctv_WP_t p_u_out_gt;
    const data_store_2d_ctv_WP_t p_v_out_gt;
    const data_store_2d_ctv_WP_t p_e_in_gt_ctv;
    const data_store_2d_ctv_WP_t ptr_coeff_1_gt_ctv;
    const data_store_2d_ctv_WP_t ptr_coeff_2_gt_ctv;
    ...
    mo_intp_rbf_rbf_vec_interpol_vertex(std::size_t VertexDim, std::size_t EdgeDim, std::size_t KDim,
        ...
        ptr_coeff_1_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(VertexDim, V2EDim).initializer([&ptr_coeff_1](int i, int j) { return ptr_coeff_1[i][j]; }).build()),
        ptr_coeff_2_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(VertexDim, V2EDim).initializer([&ptr_coeff_2](int i, int j) { return ptr_coeff_2[i][j]; }).build()),
        p_u_out_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(VertexDim, KDim).initializer([](int i, int j) { return 0; }).build()),
        p_v_out_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(VertexDim, KDim).initializer([](int i, int j) { return 0; }).build()),
        p_e_in_gt_ctv(p_e_in_gt->const_target_view()),
        ptr_coeff_1_gt_ctv(ptr_coeff_1_gt->const_target_view()),
    ...
    std::pair<std::vector<std::vector<WP_TYPE>>> get_output() {
