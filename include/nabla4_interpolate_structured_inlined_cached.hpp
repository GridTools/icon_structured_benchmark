...
__global__ void __launch_bounds__(block_dims_structured_nabla_interpol_inlined_cached_naive.size)
    ...
    // bottom
    {
        ...
        for (int i_internal_block{i}; i_internal_block < i_limit; i_internal_block += i_step) {
            ...
            const WP_TYPE inv_vert_vert_length_sqr{inv_vert_vert_length_gt_tv(edge_index) * inv_vert_vert_length_gt_tv(edge_index)};
            const WP_TYPE inv_primal_edge_length_sqr{inv_primal_edge_length_gt_tv(edge_index) * inv_primal_edge_length_gt_tv(edge_index)};
            ...
            const auto local_edge_index = i_internal_block - (blockIdx.x * blockDim.x) + 1 - halo + ((j_internal_block - (blockIdx.y * blockDim.y) + 1 - halo) * (blockDim.x + 1));
            z_nabla4_e2[local_edge_index] =
                4.0 *
                ((nabv_norm_wp - 2.0 * z_nabla2_e_gt_tv(edge_index, k_index)) * inv_vert_vert_length_sqr +
    ...
    // left
    {
        ...
        for (int j_internal_block{j}; j_internal_block < j_limit; j_internal_block += j_step) {
            ...
            const index_type E2ECV_1[2] = {E2ECV_0[0] + total_edges,
                E2ECV_0[1] + total_edges};
            const index_type E2ECV_2[2] = {E2ECV_1[0] + total_edges,
                E2ECV_1[1] + total_edges};
            const index_type E2ECV_3[2] = {E2ECV_2[0] + total_edges,
                E2ECV_2[1] + total_edges};
            const index_type edge_index = i_j;
            const WP_TYPE primal_normal_vert_v1_0[2] = {primal_normal_vert_v1_gt_tv(E2ECV_0[0]),
                primal_normal_vert_v1_gt_tv(E2ECV_0[1])};
            const WP_TYPE primal_normal_vert_v1_1[2] = {primal_normal_vert_v1_gt_tv(E2ECV_1[0]),
                primal_normal_vert_v1_gt_tv(E2ECV_1[1])};
            const WP_TYPE primal_normal_vert_v1_2[2] = {primal_normal_vert_v1_gt_tv(E2ECV_2[0]),
                primal_normal_vert_v1_gt_tv(E2ECV_2[1])};
            const WP_TYPE primal_normal_vert_v1_3[2] = {primal_normal_vert_v1_gt_tv(E2ECV_3[0]),
                primal_normal_vert_v1_gt_tv(E2ECV_3[1])};
            const WP_TYPE primal_normal_vert_v2_0[2] = {primal_normal_vert_v2_gt_tv(E2ECV_0[0]),
                primal_normal_vert_v2_gt_tv(E2ECV_0[1])};
            const WP_TYPE primal_normal_vert_v2_1[2] = {primal_normal_vert_v2_gt_tv(E2ECV_1[0]),
                primal_normal_vert_v2_gt_tv(E2ECV_1[1])};
            const WP_TYPE primal_normal_vert_v2_2[2] = {primal_normal_vert_v2_gt_tv(E2ECV_2[0]),
                primal_normal_vert_v2_gt_tv(E2ECV_2[1])};
            const WP_TYPE primal_normal_vert_v2_3[2] = {primal_normal_vert_v2_gt_tv(E2ECV_3[0]),
                primal_normal_vert_v2_gt_tv(E2ECV_3[1])};
            const WP_TYPE inv_vert_vert_length_sqr[2] = {
...
#pragma unroll
            for (auto color{0}; color < 2; ++color) {
                ...
                const auto local_edge_index = i_internal_block - (blockIdx.x * blockDim.x) + 1 - halo + ((j_internal_block - (blockIdx.y * blockDim.y) + 1 - halo) * (blockDim.x + 1)) + (color + 1) * shared_mem_inner_domain;
                z_nabla4_e2[local_edge_index] =
                    4.0 *
                    ((nabv_norm_wp - 2.0 * z_nabla2_e_gt_tv(edge_index + (color + 1) * outer_domain_size, k_index)) * inv_vert_vert_length_sqr[color] +
                        (nabv_tang_wp - 2.0 * z_nabla2_e_gt_tv(edge_index + (color + 1) * outer_domain_size, k_index)) * inv_primal_edge_length_sqr[color]);
            };
        }
    }
    for (int i_internal_block{i}; i_internal_block < i_limit; i_internal_block += i_step) {
        for (int j_internal_block{j}; j_internal_block < j_limit - 1; j_internal_block += j_step) {
            const index_type i_j = j_internal_block * x_dim + i_internal_block;
            const index_type i_jp1 = (j_internal_block + 1) * x_dim + i_internal_block;
            const index_type im1_jp1 = (j_internal_block + 1) * x_dim + i_internal_block - 1;
            const index_type ip1_j = j_internal_block * x_dim + i_internal_block + 1;
...
#pragma unroll
    ...
    // top
    {
        ...
        for (int i_internal_block{i}; i_internal_block < i_limit - 1; i_internal_block += i_step) {
            const index_type i_j = j_internal_block * x_dim + i_internal_block;
            const index_type i_jp1 = (j_internal_block + 1) * x_dim + i_internal_block;
            const index_type im1_jp1 = (j_internal_block + 1) * x_dim + i_internal_block - 1;
            const index_type ip1_j = j_internal_block * x_dim + i_internal_block + 1;
            ...
            const auto local_edge_index = i_internal_block - (blockIdx.x * blockDim.x) + 1 - halo + ((j_internal_block - (blockIdx.y * blockDim.y) + 1 - halo) * (blockDim.x + 1)) + 2 * shared_mem_inner_domain;
            z_nabla4_e2[local_edge_index] =
                4.0 *
                ((nabv_norm_wp - 2.0 * z_nabla2_e_gt_tv(edge_index + 2 * outer_domain_size, k_index)) * inv_vert_vert_length_sqr +
                    (nabv_tang_wp - 2.0 * z_nabla2_e_gt_tv(edge_index + 2 * outer_domain_size, k_index)) * inv_primal_edge_length_sqr);
        }
    }
    __syncthreads();
