...
#pragma unroll
    for (int internal_edge_id{}; internal_edge_id < 6; ++internal_edge_id) {
        ...
        const std::array<index_type, 4> e2c2v{e2c2v_gt_tv(edge_index, 0),
            ...
            e2c2v_gt_tv(edge_index, 2),
            e2c2v_gt_tv(edge_index, 3)};
        const std::array<index_type, 4> e2ecv{e2ecv_gt_tv(edge_index, 0),
            e2ecv_gt_tv(edge_index, 1),
            e2ecv_gt_tv(edge_index, 2),
            e2ecv_gt_tv(edge_index, 3)};
        const std::array<WP_TYPE, 4> primal_normal_vert_v1{primal_normal_vert_v1_gt_tv(e2ecv[0]),
            primal_normal_vert_v1_gt_tv(e2ecv[1]),
            primal_normal_vert_v1_gt_tv(e2ecv[2]),
            primal_normal_vert_v1_gt_tv(e2ecv[3])};
        const std::array<WP_TYPE, 4> primal_normal_vert_v2{primal_normal_vert_v2_gt_tv(e2ecv[0]),
        ...
        for (int k_index{initial_k_index}; k_index < KDim && k_repetition < k_repetitions; k_index += gridDim.y * blockDim.y) {
            const auto shared_mem_vertex_index{(threadIdx.x + k_repetition * blockDim.x) * 6 + internal_edge_id};
            const double nabv_tang_wp = u_vert_gt_tv(e2c2v[0], k_index) * primal_normal_vert_v1[0] +
                                        v_vert_gt_tv(e2c2v[0], k_index) * primal_normal_vert_v2[0] +
                                        u_vert_gt_tv(e2c2v[1], k_index) * primal_normal_vert_v1[1] +
                                        v_vert_gt_tv(e2c2v[1], k_index) * primal_normal_vert_v2[1];
            const double nabv_norm_wp = u_vert_gt_tv(e2c2v[2], k_index) * primal_normal_vert_v1[2] +
                                        v_vert_gt_tv(e2c2v[2], k_index) * primal_normal_vert_v2[2] +
                                        u_vert_gt_tv(e2c2v[3], k_index) * primal_normal_vert_v1[3] +
                                        v_vert_gt_tv(e2c2v[3], k_index) * primal_normal_vert_v2[3];
