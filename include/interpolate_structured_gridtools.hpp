...
class interpolate_structured : public mo_intp_rbf_rbf_vec_interpol_vertex<T> {
  ...
  private:
    const std::array<index_type, 6> get_v2e(const int i, const int j) {
        // i_j = i + j * x_dim
        // i_jm1 = i + (j - 1) * x_dim
...
#endif
                    for (auto i{0}; i < 6; ++i) {
                        u[i] = p_e_in_gt_ctv(v2e[i], k_index) * ptr_coeff_1_gt_ctv(vertex_index, i);
                        v[i] = p_e_in_gt_ctv(v2e[i], k_index) * ptr_coeff_2_gt_ctv(vertex_index, i);
                    }
