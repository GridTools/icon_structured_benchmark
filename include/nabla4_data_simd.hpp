...
struct nabla4_data_simd {
    ...
    void init_kfirst() {
        u_vert = rand_utils.random_init_vec_2d_simd<VP_TYPE>(VertexDim, std::ceil(static_cast<double>(KDim) / stdx::native_simd<VP_TYPE>::size()));
        v_vert = rand_utils.random_init_vec_2d_simd<VP_TYPE>(VertexDim, std::ceil(static_cast<double>(KDim) / stdx::native_simd<VP_TYPE>::size()));
        ...
        z_nabla2_e = rand_utils.random_init_vec_2d_simd(output_size, std::ceil(static_cast<double>(KDim) / stdx::native_simd<VP_TYPE>::size()));
    ...
    std::vector<std::vector<stdx::native_simd<D>>> convert_scalar_simd_vec(const std::vector<std::vector<D>> &vec) {
        ...
        for (index_type i{}; i < vec.size(); ++i) {
            ...
            for (index_type j{}; j < vec[i].size() % stdx::native_simd<D>::size(); ++j) {
                vec_simd[i][k_size - 1][j] = vec[i][j + (vec[i].size() / stdx::native_simd<D>::size()) * stdx::native_simd<D>::size()];
            }
        }
        return vec_simd;
    };
    ...
    nabla4_data_simd(index_type CellDim,
        ...
        std::vector<WP_TYPE> &inv_primal_edge_length)
        : CellDim(CellDim), VertexDim(VertexDim), EdgeDim(EdgeDim), KDim(KDim), ECVDim(ECVDim),
          output_size(output_size), u_vert(convert_scalar_simd_vec(u_vert)), v_vert(convert_scalar_simd_vec(v_vert)), primal_normal_vert_v1(primal_normal_vert_v1),
          primal_normal_vert_v2(primal_normal_vert_v2), z_nabla2_e(convert_scalar_simd_vec(z_nabla2_e)),
