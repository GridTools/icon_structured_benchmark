...
class nabla4_unstructured_gt : public nabla4_gt_data<T> {
  ...
  public:
    ...
    using neighbors_gt_t =
        decltype(gridtools::storage::builder<T>.dimensions(4_c, 0).template type<index_type>().build());
    using neighbors_gt_ctv_t =
        decltype(gridtools::storage::builder<T>.dimensions(4_c, 0).template type<index_type>().build()->const_target_view());
#else
    using neighbors_gt_t =
...
#if defined(__CUDACC__)
    index_type *e2c2v_gt_t_ptr;
    index_type *e2ecv_gt_t_ptr;
#endif
    ...
    nabla4_unstructured_gt(std::vector<std::array<index_type, 4>> e2c2v,
        ...
        index_type ECVDim)
#if defined(__CUDACC__)
        : e2c2v_gt(storage::builder<T>.template type<index_type>().dimensions(4_c, e2c2v.size()).initializer([&e2c2v](int i, int j) { return e2c2v[j][i]; }).build()),
        e2ecv_gt(storage::builder<T>.template type<index_type>().dimensions(4_c, e2ecv.size()).initializer([&e2ecv](int i, int j) { return e2ecv[j][i]; }).build()),
        e2c2v_gt_t_ptr(e2c2v_gt->get_target_ptr()),
        e2ecv_gt_t_ptr(e2ecv_gt->get_target_ptr()),
#else
...
inline void nabla4_unstructured_gt<T>::run_gpu_helper() {
    ...
    // std::endl; for (int i{0}; i < e2c2v_gt->const_host_view().lengths()[1]; ++i) {
    //     for (int j{0}; j < e2c2v_gt->const_host_view().lengths()[0]; ++j) {
    //         printf("%lu : %p ", e2c2v_gt_h_ptr[i * e2c2v_gt->const_host_view().strides()[1] + j *
    //         e2c2v_gt->const_host_view().strides()[0]], e2c2v_gt_h_ptr + i * e2c2v_gt->const_host_view().strides()[1]
    //         + j * e2c2v_gt->const_host_view().strides()[0]);
    //     };
    //     std::cout << std::endl;
    // };
