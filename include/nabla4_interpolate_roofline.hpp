...
template <typename T>
constexpr block_dims get_block_dims_nabla_interpol_roofline_kloop() {
    throw std::runtime_error("Undefined block dimensions for type " + T::name + " in GPU backend");
};

template <>
constexpr block_dims get_block_dims_nabla_interpol_roofline_kloop<std::size_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_nabla_interpol_roofline_kloop<std::int64_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_nabla_interpol_roofline_kloop<std::uint32_t>() {
    // not optimized
    return {32, 9, 1, 288};
};

template <>
constexpr block_dims get_block_dims_nabla_interpol_roofline_kloop<int>() {
    return {32, 4, 2, 256};
};

constexpr block_dims block_dims_nabla_interpol_roofline_kloop =
    get_block_dims_nabla_interpol_roofline_kloop<index_type>();

__global__ void __launch_bounds__(block_dims_nabla_interpol_roofline_kloop.size)
    run_gpu_kloop_nabla4_interpolate_roofline(index_type KDim,
        index_type x_dim,
        index_type y_dim,
        index_type halo,
    ...
    const std::array<WP_TYPE, 12> primal_normal_vert_v1{primal_normal_vert_v1_gt_tv(vertex_index_internal),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 2 * inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 3 * inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 4 * inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 5 * inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 6 * inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 7 * inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 8 * inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 9 * inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 10 * inner_domain_size),
        primal_normal_vert_v1_gt_tv(vertex_index_internal + 11 * inner_domain_size)};
    const std::array<WP_TYPE, 12> primal_normal_vert_v2{primal_normal_vert_v2_gt_tv(vertex_index_internal),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 2 * inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 3 * inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 4 * inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 5 * inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 6 * inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 7 * inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 8 * inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 9 * inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 10 * inner_domain_size),
        primal_normal_vert_v2_gt_tv(vertex_index_internal + 11 * inner_domain_size)};
    const std::array<WP_TYPE, 3> inv_vert_vert_length{inv_vert_vert_length_gt_tv(vertex_index_internal),
        inv_vert_vert_length_gt_tv(vertex_index_internal + inner_domain_size),
        inv_vert_vert_length_gt_tv(vertex_index_internal + 2 * inner_domain_size)};
    const std::array<WP_TYPE, 3> inv_primal_edge_length{inv_primal_edge_length_gt_tv(vertex_index_internal),
        inv_primal_edge_length_gt_tv(vertex_index_internal + inner_domain_size),
        inv_primal_edge_length_gt_tv(vertex_index_internal + 2 * inner_domain_size)};
...
template <typename T>
inline void nabla4_interpolate_roofline<T>::run_gpu_kloop_helper() {
    dim3 tblocks(block_dims_nabla_interpol_roofline_kloop.x,
        block_dims_nabla_interpol_roofline_kloop.y,
        block_dims_nabla_interpol_roofline_kloop.z);
    ...
    run_gpu_kloop_nabla4_interpolate_roofline<<<grid, tblocks>>>(interpolate_data.KDim,
...
template <typename T>
constexpr block_dims get_block_dims_nabla_interpol_roofline_naive() {
    throw std::runtime_error("Undefined block dimensions for type " + T::name + " in GPU backend");
};

template <>
constexpr block_dims get_block_dims_nabla_interpol_roofline_naive<std::size_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_nabla_interpol_roofline_naive<std::int64_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_nabla_interpol_roofline_naive<std::uint32_t>() {
    // not optimized
    return {32, 9, 1, 288};
};

template <>
constexpr block_dims get_block_dims_nabla_interpol_roofline_naive<int>() {
    return {32, 8, 2, 256};
};

constexpr block_dims block_dims_nabla_interpol_roofline_naive =
    get_block_dims_nabla_interpol_roofline_naive<index_type>();

__global__ void
#if __CUDACC_VER_MAJOR__ < 12 || (__CUDACC_VER_MAJOR__ == 12 && __CUDACC_VER_MINOR__ < 5)
__launch_bounds__(block_dims_nabla_interpol_roofline_naive.size)
#else
__maxnreg__(62)
#endif
    run_gpu_naive_nabla4_interpolate_roofline(index_type KDim,
        index_type x_dim,
        index_type y_dim,
        index_type halo,
        index_type inner_domain_size,
...
template <typename T>
inline void nabla4_interpolate_roofline<T>::run_gpu_naive_helper() {
    dim3 tblocks(block_dims_nabla_interpol_roofline_naive.x,
        block_dims_nabla_interpol_roofline_naive.y,
        block_dims_nabla_interpol_roofline_naive.z);
    const index_type inner_domain_size =
        (interpolate_data.x_dim - 2 * interpolate_data.halo) * (interpolate_data.y_dim - 2 * interpolate_data.halo);
    const index_type outer_domain_size = interpolate_data.x_dim * interpolate_data.y_dim;
    const index_type inner_x_dim = interpolate_data.x_dim - 2 * interpolate_data.halo;
    ...
    run_gpu_naive_nabla4_interpolate_roofline<<<grid, tblocks>>>(interpolate_data.KDim,
        interpolate_data.x_dim,
        interpolate_data.y_dim,
        interpolate_data.halo,
        inner_domain_size,
