...
__global__ void __launch_bounds__(block_dims_structured.size) run_gpu(index_type KDim,
    ...
    const unsigned int clusterBlockRank = cluster.block_rank();
    const auto cluster_size = cluster.dim_blocks().z;
    const auto cluster_index = cluster.block_index().z;
    printf("tid: %d, Cluster block rank: %u, cluster block size: %u, thread global index: (%d, %d, %d)\n",
        tid,
        clusterBlockRank,
        cluster_size,
    ...
    __shared__ index_type E2C2V[3 * block_dims_structured.x * 4];
    ...
    if (blockIdx.z == 0 && threadIdx.z == 0 && clusterBlockRank == 0) {
        for (auto edge_index{blockIdx.x * blockDim.x + threadIdx.x}; edge_index < total_grid_size;
            ...
            if (threadIdx.y == 0) {
                ...
                const auto im1_jp1 = (j + 1) * x_dim + i - 1;
                const auto ip1_j = j * x_dim + i + 1;
                E2C2V[0 * blockDim.x * 4 + threadIdx.x * 4 + 0] = i_j;
                E2C2V[0 * blockDim.x * 4 + threadIdx.x * 4 + 1] = i_jp1;
                E2C2V[0 * blockDim.x * 4 + threadIdx.x * 4 + 2] = im1_jp1;
                E2C2V[0 * blockDim.x * 4 + threadIdx.x * 4 + 3] = ip1_j;
            } else if (threadIdx.y == 1) {
                const auto ip1_j = j * x_dim + i + 1;
                const auto i_jp1 = (j + 1) * x_dim + i;
                const auto ip1_jm1 = (j - 1) * x_dim + i + 1;
                E2C2V[1 * blockDim.x * 4 + threadIdx.x * 4 + 0] = i_j;
                E2C2V[1 * blockDim.x * 4 + threadIdx.x * 4 + 1] = ip1_j;
                E2C2V[1 * blockDim.x * 4 + threadIdx.x * 4 + 2] = i_jp1;
                E2C2V[1 * blockDim.x * 4 + threadIdx.x * 4 + 3] = ip1_jm1;
            } else {
                const auto ip1_jm1 = (j - 1) * x_dim + i + 1;
                const auto ip1_j = j * x_dim + i + 1;
                const auto i_jm1 = (j - 1) * x_dim + i;
                E2C2V[2 * blockDim.x * 4 + threadIdx.x * 4 + 0] = i_j;
                E2C2V[2 * blockDim.x * 4 + threadIdx.x * 4 + 1] = ip1_jm1;
                E2C2V[2 * blockDim.x * 4 + threadIdx.x * 4 + 2] = ip1_j;
                E2C2V[2 * blockDim.x * 4 + threadIdx.x * 4 + 3] = i_jm1;
            };
        };
    };
    cluster.sync();
    index_type *dst_E2C2V = cluster.map_shared_rank(E2C2V, 0);
    index_type *dst_is = cluster.map_shared_rank(is, 0);
    index_type *dst_js = cluster.map_shared_rank(js, 0);
    ...
    for (auto k_index{blockIdx.z * blockDim.z + threadIdx.z}; k_index < KDim; k_index += blockDim.z * gridDim.z) {
        for (auto orientation_id{blockIdx.y * blockDim.y + threadIdx.y}; orientation_id < 3;
            ...
            for (auto edge_index{blockIdx.x * blockDim.x + threadIdx.x}; edge_index < total_grid_size;
                 edge_index += blockDim.x * gridDim.x) {
                const auto E2C2V_0 = dst_E2C2V[orientation_id * blockDim.x * 4 + threadIdx.x * 4 + 0];
                const auto E2C2V_1 = dst_E2C2V[orientation_id * blockDim.x * 4 + threadIdx.x * 4 + 1];
                const auto E2C2V_2 = dst_E2C2V[orientation_id * blockDim.x * 4 + threadIdx.x * 4 + 2];
                const auto E2C2V_3 = dst_E2C2V[orientation_id * blockDim.x * 4 + threadIdx.x * 4 + 3];
                const auto i = dst_is[threadIdx.x];
                const auto j = dst_js[threadIdx.x];
...
inline void nabla4_structured_torus_halo_gt<T>::run_gpu_helper() {
    ...
    dim3 grid((total_grid_size + tblocks.x - 1) / tblocks.x, 1, 4);
    printf("Grid dimensions: %d %d %d\n", grid.x, grid.y, grid.z);
    cudaLaunchConfig_t config = {0};
    config.gridDim = grid;
    config.blockDim = tblocks;
    ...
    void (*run_gpu_func)(index_type KDim,
        index_type x_dim,
        index_type y_dim,
        index_type halo,
        index_type total_grid_size,
        ...
        nabla4_structured_torus_halo_gt<storage::gpu>::data_store_1d_ctv_WP_t inv_vert_vert_length_gt_tv,
        nabla4_structured_torus_halo_gt<storage::gpu>::data_store_1d_ctv_WP_t inv_primal_edge_length_gt_tv,
        nabla4_structured_torus_halo_gt<storage::gpu>::data_store_2d_tv_VP_t z_nabla4_e2_wp_gt_tv) = run_gpu;
