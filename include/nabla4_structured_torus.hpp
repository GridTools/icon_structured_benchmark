...
class nabla4_structured_torus {
  ...
  public:
    ...
    nabla4_structured_torus(std::size_t CellDim,
        ...
        std::vector<double> &inv_primal_edge_length)
        : CellDim(CellDim), VertexDim(VertexDim), EdgeDim(EdgeDim), KDim(KDim), ECVDim(ECVDim),
          longitude_dim(longitude_dim), latitude_dim(latitude_dim), u_vert(u_vert), v_vert(v_vert),
    ...
    inline std::array<ARRAY_TYPE, 4> get_e2c2v_vertices_east_edge(std::size_t edge_index,
        ...
        e2c2v_ret[3] =
            ((((latitude == latitude_dim - 1) * (latitude_dim / 2)) + longitude - 1) % longitude_dim) * latitude_dim +
            latitude_p1;
        return e2c2v_ret;
    }
