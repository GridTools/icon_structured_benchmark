...
template <typename T>
constexpr block_dims get_block_dims_structured_nabla_interpol_v2c_inlined_kloop() {
    throw std::runtime_error("Undefined block dimensions for type " + T::name + " in GPU backend");
};

template <>
constexpr block_dims get_block_dims_structured_nabla_interpol_v2c_inlined_kloop<std::size_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_structured_nabla_interpol_v2c_inlined_kloop<std::int64_t>() {
    // not optimized
    return {32, 8, 1, 256};
};

template <>
constexpr block_dims get_block_dims_structured_nabla_interpol_v2c_inlined_kloop<std::uint32_t>() {
    // not optimized
    return {32, 9, 1, 288};
};

template <>
constexpr block_dims get_block_dims_structured_nabla_interpol_v2c_inlined_kloop<int>() {
    return {32, 2, 4, 256};
};
...
#pragma unroll 36
        for (int edge_index{}; edge_index < 36; ++edge_index) {
            ...
            const double nabv_tang_wp =
                u_vert_gt_tv(e2c2v[4 * edge_index], k_index) * primal_normal_vert_v1_gt_tv(edge_id) +
                v_vert_gt_tv(e2c2v[4 * edge_index], k_index) * primal_normal_vert_v2_gt_tv(edge_id) +
                u_vert_gt_tv(e2c2v[4 * edge_index + 1], k_index) *
                    primal_normal_vert_v1_gt_tv(edge_id + outer_domain_size * 3) +
                v_vert_gt_tv(e2c2v[4 * edge_index + 1], k_index) *
                    primal_normal_vert_v2_gt_tv(edge_id + outer_domain_size * 3);
            const double nabv_norm_wp = u_vert_gt_tv(e2c2v[4 * edge_index + 2], k_index) *
                                            primal_normal_vert_v1_gt_tv(edge_id + outer_domain_size * 6) +
                                        v_vert_gt_tv(e2c2v[4 * edge_index + 2], k_index) *
                                            primal_normal_vert_v2_gt_tv(edge_id + outer_domain_size * 6) +
                                        u_vert_gt_tv(e2c2v[4 * edge_index + 3], k_index) *
                                            primal_normal_vert_v1_gt_tv(edge_id + outer_domain_size * 9) +
                                        v_vert_gt_tv(e2c2v[4 * edge_index + 3], k_index) *
                                            primal_normal_vert_v2_gt_tv(edge_id + outer_domain_size * 9);
...
#pragma unroll 6
        for (int vertex_index{}; vertex_index < 6; ++vertex_index) {
            ...
            p_u_out[vertex_index] =
                ...
                z_nabla4_e2[vertex_index * 6 + 2] * ptr_coeff_1[2] +
                z_nabla4_e2[vertex_index * 6 + 3] * ptr_coeff_1[3] +
            ...
            p_v_out[vertex_index] =
                ...
                z_nabla4_e2[vertex_index * 6 + 2] * ptr_coeff_2[2] +
...
inline void nabla4_interpolate_verts2cells_structured_inlined<T>::run_gpu_kloop_helper() {
    constexpr dim3 tblocks(block_dims_structured_nabla_interpol_v2c_inlined_kloop.x,
    ...
    dim3 grid((verts2cells_output_x_dim + tblocks.x - 1) / tblocks.x,
        ...
        1);
    run_gpu_kloop_nabla4_interpolate_verts2cells_inlined_structured<<<grid, tblocks>>>(verts2cells_data.KDim,
