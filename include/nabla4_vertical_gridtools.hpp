...
template <typename T>
struct nabla4_vertical_gt_data {
    std::size_t CellDim;
    ...
    const data_store_2d_WP_t primal_normal_vert_v1_gt;
    const data_store_2d_WP_t primal_normal_vert_v2_gt;
    const data_store_2d_WP_t z_nabla2_e_gt;
    const data_store_2d_WP_t inv_vert_vert_length_gt;
    const data_store_2d_WP_t inv_primal_edge_length_gt;
    ...
    const data_store_2d_ctv_WP_t primal_normal_vert_v1_gt_tv;
    const data_store_2d_ctv_WP_t primal_normal_vert_v2_gt_tv;
    const data_store_2d_ctv_WP_t z_nabla2_e_gt_tv;
    const data_store_2d_ctv_WP_t inv_vert_vert_length_gt_tv;
    const data_store_2d_ctv_WP_t inv_primal_edge_length_gt_tv;
    const data_store_2d_tv_VP_t z_nabla4_e2_wp_gt_tv;

    nabla4_vertical_gt_data(std::size_t CellDim, std::size_t VertexDim, std::size_t EdgeDim, std::size_t KDim, std::size_t ECVDim, std::size_t output_size)
        ...
        primal_normal_vert_v1_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(EdgeDim * ECVDim, KDim).initializer([](int i, int j) { return rand_utils.template get<WP_TYPE>(); }).build()),
        primal_normal_vert_v2_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(EdgeDim * ECVDim, KDim).initializer([](int i, int j) { return rand_utils.template get<WP_TYPE>(); }).build()),
        ...
        inv_vert_vert_length_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(output_size, KDim).initializer([](int i, int j) { return rand_utils.template get<WP_TYPE>(); }).build()),
        inv_primal_edge_length_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(output_size, KDim).initializer([](int i, int j) { return rand_utils.template get<WP_TYPE>(); }).build()),
    ...
    nabla4_vertical_gt_data(std::size_t CellDim, std::size_t VertexDim, std::size_t EdgeDim, std::size_t KDim, std::size_t ECVDim, std::size_t output_size, std::vector<std::vector<VP_TYPE>> &u_vert,
        ...
        primal_normal_vert_v1_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(EdgeDim * ECVDim, KDim).initializer([&primal_normal_vert_v1](int i, int j) { return primal_normal_vert_v1[i]; }).build()),
        primal_normal_vert_v2_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(EdgeDim * ECVDim, KDim).initializer([&primal_normal_vert_v2](int i, int j) { return primal_normal_vert_v2[i]; }).build()),
        ...
        inv_vert_vert_length_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(output_size, KDim).initializer([&inv_vert_vert_length](int i, int j) { return inv_vert_vert_length[i]; }).build()),
        inv_primal_edge_length_gt(storage::builder<T>.template type<WP_TYPE>().dimensions(output_size, KDim).initializer([&inv_primal_edge_length](int i, int j) { return inv_primal_edge_length[i]; }).build()),
        z_nabla4_e2_wp_gt(storage::builder<T>.template type<VP_TYPE>().dimensions(output_size, KDim).initializer([](int i, int j) { return 0; }).build()),
        u_vert_gt_tv(u_vert_gt->const_target_view()),
        v_vert_gt_tv(v_vert_gt->const_target_view()),
        primal_normal_vert_v1_gt_tv(primal_normal_vert_v1_gt->const_target_view()),
    ...
    data_store_2d_VP_t get_output_gt() { return z_nabla4_e2_wp_gt; }
};
