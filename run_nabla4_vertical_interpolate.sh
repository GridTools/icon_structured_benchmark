...
for commit in $COMMITS; do
    ...
    OUTPUT_DIR="nabla4_vertical_interpolate_output_${commit}_${BACKEND}_${INDEX_TYPE}${SUFFIX}"
    ...
    for grid_file in ${GRID_FILES}; do
        ...
        for k in $KLEVELS; do
            FILE_NAME="${grid_name}_k${k}_${commit}_${BACKEND}_${INDEX_TYPE}_vertical"
            # Make sure that libnvrtc.so is in the LD_LIBRARY_PATH
            LD_LIBRARY_PATH="<PATH_TO_libnvrtc.so>":$LD_LIBRARY_PATH python3.11 run_filtered_torus_grid_int_nabla4_interpolate.py ${grid_file} --klevels $k --repetitions $REPETITIONS --output ${FILE_NAME}.json --backend all_gpu --dry-run --vertical
            if [ $? -ne 0 ]; then
