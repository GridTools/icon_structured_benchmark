...
std::vector<std::vector<VP_TYPE>> nabla4_validate_vector(std::tuple<Args...> &&args) {
    if constexpr (I == backend_impl::naive) {
        ...
        return run_validation<T<Data::ifirst>, I>(benchmark_object);
    } else if constexpr (I == backend_impl::cpu_ifirst) {
        T<Data::ifirst> benchmark_object{
            std::apply([](auto &&...args) { return T<Data::ifirst>{std::forward<decltype(args)>(args)...}; }, args)};
        return run_validation<T<Data::ifirst>, I>(benchmark_object);
        ...
        return run_validation<T<Data::kfirst>, I>(benchmark_object);
    } else if constexpr (I == backend_impl::cpu_kfirst_pragma) {
        T<Data::kfirst> benchmark_object{
            std::apply([](auto &&...args) { return T<Data::kfirst>{std::forward<decltype(args)>(args)...}; }, args)};
        return run_validation<T<Data::kfirst>, I>(benchmark_object);
...
std::vector<double> nabla4_benchmark_unstructured_cpu_kfirst_pragma(std::vector<std::array<std::size_t, 4>> &e2c2v,
    ...
    int repetitions,
    int dry_runs) {
    return nabla4_benchmark_vector<cpu_kfirst_pragma, nabla4_unstructured>(
        std::make_tuple(e2c2v, e2ecv, CellDim, VertexDim, EdgeDim, KDim, ECVDim), repetitions, dry_runs);
}
...
std::vector<double> nabla4_benchmark_unstructured_cpu_kfirst_simd(std::vector<std::array<std::size_t, 4>> &e2c2v,
    ...
    int repetitions,
    int dry_runs) {
    return nabla4_benchmark_vector<cpu_kfirst_simd, nabla4_unstructured>(
        std::make_tuple(e2c2v, e2ecv, CellDim, VertexDim, EdgeDim, KDim, ECVDim), repetitions, dry_runs);
}
...
std::vector<double> nabla4_benchmark_unstructured_simd_cpu_kfirst_simd(std::vector<std::array<std::size_t, 4>> &e2c2v,
    ...
    int repetitions,
    int dry_runs) {
    return nabla4_benchmark_vector<cpu_kfirst_simd, nabla4_unstructured_simd>(
        std::make_tuple(e2c2v, e2ecv, CellDim, VertexDim, EdgeDim, KDim, ECVDim), repetitions, dry_runs);
}
...
std::vector<std::vector<VP_TYPE>> nabla4_validate_unstructured_cpu_kfirst_pragma(
    ...
    std::vector<WP_TYPE> &inv_vert_vert_length,
    std::vector<WP_TYPE> &inv_primal_edge_length) {
    return nabla4_validate_vector<cpu_kfirst_pragma, nabla4_unstructured>(std::make_tuple(e2c2v,
        e2ecv,
        CellDim,
        VertexDim,
        EdgeDim,
...
std::vector<std::vector<VP_TYPE>> nabla4_validate_unstructured_cpu_kfirst_simd(
    ...
    std::vector<WP_TYPE> &inv_vert_vert_length,
    std::vector<WP_TYPE> &inv_primal_edge_length) {
    return nabla4_validate_vector<cpu_kfirst_simd, nabla4_unstructured>(std::make_tuple(e2c2v,
        e2ecv,
        CellDim,
        VertexDim,
        EdgeDim,
...
std::vector<std::vector<VP_TYPE>> nabla4_validate_unstructured_simd_cpu_kfirst_simd(
    ...
    std::vector<WP_TYPE> &inv_vert_vert_length,
    std::vector<WP_TYPE> &inv_primal_edge_length) {
    return nabla4_validate_vector<cpu_kfirst_simd, nabla4_unstructured_simd>(std::make_tuple(e2c2v,
        e2ecv,
        CellDim,
        VertexDim,
        EdgeDim,
