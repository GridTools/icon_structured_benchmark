...
def run_gtfn(e2c2v, e2ecv, nabla4_data, grid, backend):
    if backend == "gt:gpu":
    ...
    z_nabla4_e2_wp_gtfn = zeros(
        shape=(len(e2c2v), grid.num_levels), dtype=ds_dtype, backend=backend
    )
    if backend == "gt:gpu":
        runtime = nabla4_gtfn.calculate_nabla4_gpu(
            (
                from_array(
                    np.array(nabla4_data.u_vert).T, dtype=ds_dtype, backend=backend
                ),
                (0, 0),
            ),
            (
                from_array(
                    np.array(nabla4_data.v_vert).T, dtype=ds_dtype, backend=backend
                ),
                (0, 0),
            ),
            (
            ...
            grid.num_levels,
            (
                from_array(np.array(e2c2v), dtype=ds_dtype, backend=backend),
                (0, 0),
            ),
            (
                from_array(np.array(e2ecv), dtype=ds_dtype, backend=backend),
                (0, 0),
            ),
        )
    else:
...
def run_sanity_checks(
    ...
    if backend in ["all_gpu", "gtfn_gpu"]:
        ...
        z_nabla4_e2_wp_gtfn, _ = run_gtfn(
            filtered_e2c2v, filtered_e2ecv, random_validation_data, grid, "gt:gpu"
        )
...
def run_benchmarks():
    ...
    if args.backend in ["all_cpu", "gtfn_cpu"]:
        ...
        for _ in range(repetitions):
            ...
            runtimes["nabla4_benchmark_unstructured_gtfn_cpu"].append(runtime)
