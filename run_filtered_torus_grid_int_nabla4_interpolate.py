...
def run_sanity_checks(
    ...
    if backend in ["all_gpu", "gpu_kloop"]:
        ...
        if combination in ["all", "inlined"]:
            ...
            (
                p_u_out_gpu_kloop_unstructured_inlined_vertical,
                p_v_out_gpu_kloop_unstructured_inlined_vertical,
            ) = icon_benchmark.nabla4_vertical_interpolate_validate_unstructured_gpu_kloop_inlined_cached(
                filtered_e2c2v_inlined,
                filtered_e2ecv_inlined,
                filtered_e2v_inlined,
                random_validation_data_separate.CellDim,
            ...
            assert np.allclose(
                p_u_out_gpu_kloop_unstructured_inlined_vertical, p_u_out_ref_separate
            )
            assert np.allclose(
                p_v_out_gpu_kloop_unstructured_inlined_vertical, p_v_out_ref_separate
            )
            ...
            if not np.allclose(
                ...
                print(p_u_out_gpu_kloop_unstructured_inlined_cached[0:5])
                print("p_u_out_ref_separate")
                print(p_u_out_ref_separate[0:5])
            assert np.allclose(
                p_u_out_gpu_kloop_unstructured_inlined_cached, p_u_out_ref_separate
            )
            assert np.allclose(
